...
def matriculado(alunos, matricula):
    for dados in alunos:
        for informacao in dados:
            if informacao == 'Matricula' and dados[informacao].upper() == matricula:
                return True

    return False

def guardar_dados(alunos):
    arquivo = open('Alunos.txt', mode='w')
    for dados in alunos:
        for infomacao in dados:
            arquivo.write(dados[infomacao]+'!')
        arquivo.write('\n')
    arquivo.close()
...
def iniciar_programa():
    if os.path.isfile('Alunos.txt'):
        ...
        dados_alunos = []
        dados = open('Alunos.txt').readlines()

        for informacao in dados:
            aluno = {}
            informacao.strip()
            aluno['Nome'] = informacao.split('!')[0]
            aluno['Matricula'] = informacao.split('!')[1].upper()

            if matriculado(dados_alunos, aluno['Matricula']):
                print('\nOs dados salvos apresentam matriculas iguais. Apenas uma sera lida.')
                aluno = {}
                input('Continuar...')
                apagar_tela()
            else:
                aluno['Curso'] = informacao.split('!')[2]
